package main

import (
	"context"
	"encoding/json"
	"io"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
)

func newTestServer(handler http.HandlerFunc) (*httptest.Server, *ServiceClients) {
	srv := httptest.NewServer(handler)
	cfg := &Config{
		PPTAgentURL:  srv.URL,
		KBServiceURL: srv.URL,
		MemoryURL:    srv.URL,
		SearchURL:    srv.URL,
		DBServiceURL: srv.URL,
	}
	return srv, NewServiceClients(cfg)
}

func apiOK(data any) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		d, _ := json.Marshal(data)
		resp := APIResponse{Code: 200, Message: "ok", Data: d}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
	}
}

func apiError(code int, msg string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		resp := APIResponse{Code: code, Message: msg}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
	}
}

// ===========================================================================
// postJSON / getJSON
// ===========================================================================

func TestPostJSON_Success(t *testing.T) {
	srv, clients := newTestServer(apiOK(map[string]string{"result": "ok"}))
	defer srv.Close()

	var out map[string]string
	err := clients.postJSON(context.Background(), srv.URL+"/test", map[string]string{"a": "b"}, &out)
	if err != nil {
		t.Fatal(err)
	}
	if out["result"] != "ok" {
		t.Errorf("result = %v", out)
	}
}

func TestPostJSON_ServerError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(500)
		w.Write([]byte("internal error"))
	}))
	defer srv.Close()
	clients := NewServiceClients(&Config{PPTAgentURL: srv.URL})

	err := clients.postJSON(context.Background(), srv.URL+"/test", nil, nil)
	if err == nil {
		t.Fatal("expected error for 500")
	}
}

func TestPostJSON_APIError(t *testing.T) {
	srv, clients := newTestServer(apiError(50200, "service unavailable"))
	defer srv.Close()

	var out map[string]string
	err := clients.postJSON(context.Background(), srv.URL+"/test", nil, &out)
	if err == nil {
		t.Fatal("expected error for API error code")
	}
}

func TestPostJSON_NilOut(t *testing.T) {
	srv, clients := newTestServer(apiOK(nil))
	defer srv.Close()

	err := clients.postJSON(context.Background(), srv.URL+"/test", nil, nil)
	if err != nil {
		t.Fatal(err)
	}
}

func TestGetJSON_Success(t *testing.T) {
	srv, clients := newTestServer(apiOK(UserProfile{UserID: "u1", DisplayName: "Test"}))
	defer srv.Close()

	var out UserProfile
	err := clients.getJSON(context.Background(), srv.URL+"/test", &out)
	if err != nil {
		t.Fatal(err)
	}
	if out.UserID != "u1" {
		t.Errorf("user_id = %q", out.UserID)
	}
}

func TestGetJSON_ServerError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(404)
	}))
	defer srv.Close()
	clients := NewServiceClients(&Config{PPTAgentURL: srv.URL})

	var out UserProfile
	err := clients.getJSON(context.Background(), srv.URL+"/test", &out)
	if err == nil {
		t.Fatal("expected error for 404")
	}
}

// ===========================================================================
// QueryKB
// ===========================================================================

func TestQueryKB(t *testing.T) {
	srv, clients := newTestServer(apiOK(KBQueryResponse{
		Chunks: []RetrievedChunk{{ChunkID: "c1", Content: "data", Score: 0.9}},
		Total:  1,
	}))
	defer srv.Close()

	resp, err := clients.QueryKB(context.Background(), KBQueryRequest{
		UserID: "u1", Query: "test", TopK: 5,
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(resp.Chunks) != 1 {
		t.Errorf("chunks = %d", len(resp.Chunks))
	}
}

// ===========================================================================
// RecallMemory
// ===========================================================================

func TestRecallMemory(t *testing.T) {
	srv, clients := newTestServer(apiOK(MemoryRecallResponse{
		ProfileSummary: "summary",
	}))
	defer srv.Close()

	resp, err := clients.RecallMemory(context.Background(), MemoryRecallRequest{
		UserID: "u1", Query: "test",
	})
	if err != nil {
		t.Fatal(err)
	}
	if resp.ProfileSummary != "summary" {
		t.Errorf("summary = %q", resp.ProfileSummary)
	}
}

// ===========================================================================
// GetUserProfile
// ===========================================================================

func TestGetUserProfile(t *testing.T) {
	srv, clients := newTestServer(apiOK(UserProfile{UserID: "u1", Subject: "数学"}))
	defer srv.Close()

	profile, err := clients.GetUserProfile(context.Background(), "u1")
	if err != nil {
		t.Fatal(err)
	}
	if profile.Subject != "数学" {
		t.Errorf("subject = %q", profile.Subject)
	}
}

// ===========================================================================
// SearchWeb
// ===========================================================================

func TestSearchWeb(t *testing.T) {
	srv, clients := newTestServer(apiOK(SearchResponse{
		Summary: "found results",
		Results: []SearchResult{{Title: "Result1"}},
	}))
	defer srv.Close()

	resp, err := clients.SearchWeb(context.Background(), SearchRequest{
		UserID: "u1", Query: "test",
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(resp.Results) != 1 {
		t.Errorf("results = %d", len(resp.Results))
	}
}

// ===========================================================================
// InitPPT
// ===========================================================================

func TestInitPPT(t *testing.T) {
	srv, clients := newTestServer(apiOK(PPTInitResponse{TaskID: "task_new"}))
	defer srv.Close()

	resp, err := clients.InitPPT(context.Background(), PPTInitRequest{
		UserID: "u1", Topic: "数学",
	})
	if err != nil {
		t.Fatal(err)
	}
	if resp.TaskID != "task_new" {
		t.Errorf("task_id = %q", resp.TaskID)
	}
}

// ===========================================================================
// SendFeedback
// ===========================================================================

func TestSendFeedback(t *testing.T) {
	srv, clients := newTestServer(apiOK(nil))
	defer srv.Close()

	err := clients.SendFeedback(context.Background(), PPTFeedbackRequest{
		TaskID: "t1", RawText: "修改内容",
	})
	if err != nil {
		t.Fatal(err)
	}
}

// ===========================================================================
// GetCanvasStatus
// ===========================================================================

func TestGetCanvasStatus(t *testing.T) {
	srv, clients := newTestServer(apiOK(CanvasStatusResponse{
		TaskID:    "t1",
		PageOrder: []string{"p1"},
	}))
	defer srv.Close()

	resp, err := clients.GetCanvasStatus(context.Background(), "t1")
	if err != nil {
		t.Fatal(err)
	}
	if resp.TaskID != "t1" {
		t.Errorf("task_id = %q", resp.TaskID)
	}
}

// ===========================================================================
// UploadFile
// ===========================================================================

func TestUploadFile_NilBody(t *testing.T) {
	clients := NewServiceClients(&Config{DBServiceURL: "http://localhost:1"})
	r := httptest.NewRequest(http.MethodPost, "/upload", nil)
	r.Body = nil
	_, err := clients.UploadFile(r)
	if err == nil {
		t.Fatal("expected error for nil body")
	}
}

func TestUploadFile_Success(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		if len(body) == 0 {
			t.Error("empty body received")
		}
		resp := APIResponse{
			Code:    200,
			Message: "ok",
			Data:    json.RawMessage(`{"file_id":"f_uploaded"}`),
		}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
	}))
	defer srv.Close()
	clients := NewServiceClients(&Config{DBServiceURL: srv.URL})

	r := httptest.NewRequest(http.MethodPost, "/upload", strings.NewReader("fake file data"))
	r.Header.Set("Content-Type", "multipart/form-data")
	r.Header.Set("Authorization", "Bearer token123")

	data, err := clients.UploadFile(r)
	if err != nil {
		t.Fatal(err)
	}
	var result map[string]string
	json.Unmarshal(data, &result)
	if result["file_id"] != "f_uploaded" {
		t.Errorf("file_id = %q", result["file_id"])
	}
}

func TestUploadFile_UpstreamError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(500)
		w.Write([]byte("error"))
	}))
	defer srv.Close()
	clients := NewServiceClients(&Config{DBServiceURL: srv.URL})

	r := httptest.NewRequest(http.MethodPost, "/upload", strings.NewReader("data"))
	_, err := clients.UploadFile(r)
	if err == nil {
		t.Fatal("expected error for 500")
	}
}

func TestUploadFile_APIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := APIResponse{Code: 40001, Message: "bad request", Data: json.RawMessage(`{"err":"detail"}`)}
		json.NewEncoder(w).Encode(resp)
	}))
	defer srv.Close()
	clients := NewServiceClients(&Config{DBServiceURL: srv.URL})

	r := httptest.NewRequest(http.MethodPost, "/upload", strings.NewReader("data"))
	_, err := clients.UploadFile(r)
	if err == nil {
		t.Fatal("expected error for API error")
	}
}

func TestUploadFile_RawResponse(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`{"file_id":"raw_response"}`))
	}))
	defer srv.Close()
	clients := NewServiceClients(&Config{DBServiceURL: srv.URL})

	r := httptest.NewRequest(http.MethodPost, "/upload", strings.NewReader("data"))
	data, err := clients.UploadFile(r)
	if err != nil {
		t.Fatal(err)
	}
	if string(data) != `{"file_id":"raw_response"}` {
		t.Errorf("data = %s", string(data))
	}
}

// ===========================================================================
// ExtractMemory
// ===========================================================================

func TestExtractMemory(t *testing.T) {
	srv, clients := newTestServer(apiOK(MemoryExtractResponse{
		ExtractedFacts: []string{"fact1"},
	}))
	defer srv.Close()

	resp, err := clients.ExtractMemory(context.Background(), MemoryExtractRequest{
		UserID: "u1", SessionID: "s1",
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(resp.ExtractedFacts) != 1 {
		t.Errorf("facts = %d", len(resp.ExtractedFacts))
	}
}

// ===========================================================================
// SaveWorkingMemory
// ===========================================================================

func TestSaveWorkingMemory(t *testing.T) {
	srv, clients := newTestServer(apiOK(nil))
	defer srv.Close()

	err := clients.SaveWorkingMemory(context.Background(), WorkingMemorySaveRequest{
		SessionID: "s1", UserID: "u1",
	})
	if err != nil {
		t.Fatal(err)
	}
}

// ===========================================================================
// GetWorkingMemory
// ===========================================================================

func TestGetWorkingMemory(t *testing.T) {
	srv, clients := newTestServer(apiOK(WorkingMemory{
		SessionID: "s1", ConversationSummary: "summary",
	}))
	defer srv.Close()

	mem, err := clients.GetWorkingMemory(context.Background(), "s1")
	if err != nil {
		t.Fatal(err)
	}
	if mem.ConversationSummary != "summary" {
		t.Errorf("summary = %q", mem.ConversationSummary)
	}
}

// ===========================================================================
// NotifyVADEvent
// ===========================================================================

func TestNotifyVADEvent(t *testing.T) {
	var received VADEvent
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		json.NewDecoder(r.Body).Decode(&received)
		resp := APIResponse{Code: 200}
		json.NewEncoder(w).Encode(resp)
	}))
	defer srv.Close()
	clients := NewServiceClients(&Config{PPTAgentURL: srv.URL})

	err := clients.NotifyVADEvent(context.Background(), VADEvent{
		TaskID: "t1", Timestamp: 12345, ViewingPageID: "p1",
	})
	if err != nil {
		t.Fatal(err)
	}
	if received.TaskID != "t1" {
		t.Errorf("task_id = %q", received.TaskID)
	}
}

// ===========================================================================
// IngestFromSearch
// ===========================================================================

func TestIngestFromSearch(t *testing.T) {
	srv, clients := newTestServer(apiOK(nil))
	defer srv.Close()

	err := clients.IngestFromSearch(context.Background(), IngestFromSearchRequest{
		UserID: "u1",
		Items:  []SearchIngestItem{{Title: "t1", URL: "http://a.com"}},
	})
	if err != nil {
		t.Fatal(err)
	}
}

// ===========================================================================
// NewServiceClients
// ===========================================================================

func TestNewServiceClients(t *testing.T) {
	cfg := &Config{
		PPTAgentURL:  "http://ppt:9100/",
		KBServiceURL: "http://kb:9200/",
		MemoryURL:    "http://mem:9300/",
		SearchURL:    "http://search:9400/",
		DBServiceURL: "http://db:9500/",
	}
	c := NewServiceClients(cfg)
	if c.pptBaseURL != "http://ppt:9100" {
		t.Errorf("trailing slash not trimmed: %q", c.pptBaseURL)
	}
}
