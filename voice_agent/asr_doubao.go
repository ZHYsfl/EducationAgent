...
func (c *DouBaoASRClient) RecognizeStream(ctx context.Context, audioCh <-chan []byte, resultBufSize int) (<-chan ASRResult, error) {
	...
	go func() {
		audioHeader := buildHeader(msgTypeAudioOnlyReq, flagNoSeq, serNone, compNone)
		lastHeader := buildHeader(msgTypeAudioOnlyReq, flagLastData, serNone, compNone)
		...
		for {
			...
			case audio, ok := <-audioCh:
				if !ok {
					...
					frame := buildFrame(lastHeader, nil)
					if err := conn.WriteMessage(websocket.BinaryMessage, frame); err != nil {
						log.Printf("doubao asr last packet: %v", err)
					}
					return
				}
				frame := buildFrame(audioHeader, audio)
				if err := conn.WriteMessage(websocket.BinaryMessage, frame); err != nil {
	...
	go func() {
		...
		for {
			...
			if h.MsgType != msgTypeFullServerResp {
				continue
			}

			payload, _, isLast, pErr := parseServerResponse(data)
			if pErr != nil {
