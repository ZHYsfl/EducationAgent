...
func TestIsSentenceEnd(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			got := agent.IsSentenceEnd(tt.input)
			if got != tt.want {
				t.Errorf("agent.IsSentenceEnd(%q) = %v, want %v", tt.input, got, tt.want)
			}
		})
	}
}
...
func TestTruncate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			got := agent.Truncate(tt.input, tt.maxLen)
			if got != tt.want {
				t.Errorf("agent.Truncate(%q, %d) = %q, want %q", tt.input, tt.maxLen, got, tt.want)
			}
		})
	}
}
