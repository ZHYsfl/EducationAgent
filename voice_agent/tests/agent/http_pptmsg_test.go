...
func TestHandlePPTMessage_PPTStatus(t *testing.T) {
	...
	found, ok := agent.FindWSMessage(msgs, "task_status")
	if !ok {
...
func TestHandlePPTMessage_PageRendered(t *testing.T) {
	...
	time.Sleep(30 * time.Millisecond)
	msgs := agent.DrainWriteCh(s)
	found, ok := agent.FindWSMessage(msgs, "page_rendered")
	if !ok {
...
func TestHandlePPTMessage_PPTPreview(t *testing.T) {
	...
	time.Sleep(30 * time.Millisecond)
	msgs := agent.DrainWriteCh(s)
	_, ok := agent.FindWSMessage(msgs, "ppt_preview")
	if !ok {
...
func TestHandlePPTMessage_ExportReady(t *testing.T) {
	...
	time.Sleep(30 * time.Millisecond)
	msgs := agent.DrainWriteCh(s)
	found, ok := agent.FindWSMessage(msgs, "export_ready")
	if !ok {
...
func TestHandlePPTMessage_Error(t *testing.T) {
	...
	time.Sleep(30 * time.Millisecond)
	msgs := agent.DrainWriteCh(s)
	found, ok := agent.FindWSMessage(msgs, "error")
	if !ok {
