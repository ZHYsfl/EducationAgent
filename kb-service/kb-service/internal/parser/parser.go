...
func (p *SimpleParser) parseViaExternalService(ctx context.Context, input model.ParseInput) (*model.ParsedDocument, error) {
	url := p.pythonServiceURL + "/parse"
	...
	if err != nil {
		return nil, fmt.Errorf("new parse request: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")

	resp, err := p.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("parse http do: %w", err)
	}
	defer resp.Body.Close()

	respBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("read parse response: %w", err)
	}
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("parse service error %d: %s", resp.StatusCode, string(respBytes))
	}
