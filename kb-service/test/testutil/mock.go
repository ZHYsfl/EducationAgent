...
func (m *MockMetaStore) Ping(ctx context.Context) error { return nil }

// ── Mock 去重 ────────────────────────────────────────────────────────────────

func (m *MockMetaStore) FileExistsForUser(userID, fileID string) (bool, error) {
	return false, nil
}
func (m *MockMetaStore) ContentHashExists(userID, contentHash string) (bool, error) {
	return false, nil
}
func (m *MockMetaStore) RecordContentHash(userID, contentHash, docID string, createdAt int64) error {
	return nil
}

// ── Mock DLQ ────────────────────────────────────────────────────────────────

func (m *MockMetaStore) DLQPush(job store.IndexJob) error {
	return nil
}
func (m *MockMetaStore) DLQPop(count int) ([]store.IndexJob, error) {
	return nil, nil
}
func (m *MockMetaStore) DLQSize() (int, error) {
	return 0, nil
}
...
type MockVecStore struct {
	FailUpsert bool
	Upserted   []store.ChunkVector
}

func (v *MockVecStore) UpsertChunks(ctx context.Context, chunks []store.ChunkVector) error {
	if v.FailUpsert {
		return fmt.Errorf("upsert failed (mock)")
	}
	v.Upserted = append(v.Upserted, chunks...)
	return nil
}
...
func (f *FailEmbedder) Embed(_ context.Context, texts []string) ([][]float32, error) {
	return nil, fmt.Errorf("embed service unavailable (mock)")
}

// ── SleepEmbedder：延迟后成功，用于测试队列满场景 ──────────────────────────────

type SleepEmbedder struct {
	Dur time.Duration
}

func (s *SleepEmbedder) Embed(_ context.Context, texts []string) ([][]float32, error) {
	time.Sleep(s.Dur)
