...
// newRouter 构造完整测试路由（使用 MockEmbedder）
func newRouter(meta *testutil.MockMetaStore, oss *testutil.MockOSS) *gin.Engine {
	return newRouterWithEmbedder(meta, oss, &parser.MockEmbedder{})
}

// newRouterWithEmbedder 支持自定义 embedder（用于测试 embed 失败场景）
func newRouterWithEmbedder(meta *testutil.MockMetaStore, oss *testutil.MockOSS, emb parser.Embedder) *gin.Engine {
	...
	queryH := handler.NewQueryHandler(vec, emb, nil)
	...
	r.POST("/api/v1/kb/query", queryH.Query)
	r.POST("/api/v1/kb/ingest-from-search", ingestH.IngestFromSearch)
	r.POST("/api/v1/kb/parse", parseH.Parse)
	return r
}
