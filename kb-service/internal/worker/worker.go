...
// 流程：接收 IndexJob -> 解析 -> 切块 -> 向量化 -> 写入 Redis Vector -> 更新元数据
package worker
...
func (w *IndexWorker) process(job IndexJob) {
	...
	if err != nil {
		...
		_ = w.rdb.UpdateDocumentStatus(job.DocID, "failed", err.Error(), 0)
		return
	}
	...
	if len(vectors) != len(parsed.TextChunks) {
		...
		_ = w.rdb.UpdateDocumentStatus(job.DocID, "failed", msg, 0)
		return
	}

	// 4. 写入 Redis Vector（RediSearch HNSW）
	...
	// 5. 更新状态 indexed
	_ = w.rdb.UpdateDocumentStatus(job.DocID, "indexed", "", len(parsed.TextChunks))
	// 6. 集合文档计数 +1
	_ = w.rdb.IncrDocCount(job.CollectionID, time.Now().UnixMilli())
