...
| POST | `/api/v1/kb/documents` | 异步索引文档 |
...
|------|--------|------|
| `PORT` | `9200` | 服务监听端口 |
| `REDIS_ADDR` | `localhost:6379` | Redis Stack 地址（同时存元数据 + 向量索引）|
...
```bash
# 1. 启动依赖（只需要 Redis Stack，内置 RediSearch 向量索引）
...
│   ├── store/
│   │   ├── redis.go              # Redis 元数据层（集合/文档 CRUD）
│   │   └── vector.go             # Redis Stack RediSearch 向量层（替代 Qdrant）
│   ├── parser/
...
3. **文档索引触发**：用户通过前端上传文件后，文件经 Database Service 存入对象存储，由 **Voice Agent 拿到 `file_id` 和 `file_url` 后**调用 `POST /api/v1/kb/documents` 触发索引。KB Service 不直接对接 Database Service。
