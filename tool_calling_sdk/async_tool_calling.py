...
class Agent:
    ...
    def _get_tool_response_observations(self, observations: list[dict], response) -> list[dict]:
        available_functions = {
    ...
    async def chat(self, observations: list[dict]) -> list[dict]:
        ...
        while response.choices[0].finish_reason == "tool_calls":
            observations_next = self._get_tool_response_observations(observations_next, response)
            response = await self.client.chat.completions.create(
                model=self.config.model,  
