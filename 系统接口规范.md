...
| P1  | `key_difficulties`   | "有哪些重点难点需要特别强调的吗？"                |
| P1  | `duration`           | "这节课大概多长时间？"                      |
| P1  | `global_style`       | "课件风格有什么偏好吗？比如科技风、简约风、学术风？"       |
| P1  | `interaction_design` | "需要设计一些互动环节吗？比如课堂小测验、小游戏之类的？"     |
| P2  | `total_pages`        | "您希望大概多少页？还是我根据内容量自动安排？"          |
| P2  | `reference_files`    | "有参考资料需要上传吗？比如以前的教案、参考PPT、教材PDF？" |
| P2  | `additional_notes`   | "还有其他特殊要求吗？"                      |
...
5. 所有P0必填项收集完毕后，生成一份结构化摘要让教师确认
6. 确认后输出特殊标记: [REQUIREMENTS_CONFIRMED]
```
...
type PPTFeedbackRequest struct {
    ...
    ViewingPageID     string   `json:"viewing_page_id"`      // 用户开口时看的页面
    ReplyToContextID  string   `json:"reply_to_context_id"`  // 若回答 Agent 提问，填对应 context_id；否则留空
...
type KBQueryRequest struct {
    CollectionID string            `json:"collection_id"`        // 可选：为空则搜索用户所有集合
    UserID       string            `json:"user_id"`              // 必填
    Query        string            `json:"query"`                // 必填：自然语言查询
    TopK         int               `json:"top_k"`                // 返回数量，默认 5，最大 20
    ScoreThreshold float64         `json:"score_threshold"`      // 最低相似度阈值，默认 0.5
    Filters      map[string]string `json:"filters,omitempty"`    // 元数据过滤
}
```
...
type MemoryExtractRequest struct {
    UserID       string   `json:"user_id"`
    SessionID    string   `json:"session_id"`
    Messages     []ConversationTurn `json:"messages"` // 最近 N 轮对话
}
...
type MemoryRecallRequest struct {
    UserID    string `json:"user_id"`    // 必填
    SessionID string `json:"session_id"` // 可选：为空则不含工作记忆
    Query     string `json:"query"`      // 必填：当前用户的话
    TopK      int    `json:"top_k"`      // 返回数量，默认 10
}
```
...
type MemoryRecallResponse struct {
    Facts           []MemoryEntry   `json:"facts"`            // 相关事实
    Preferences     []MemoryEntry   `json:"preferences"`      // 相关偏好
    WorkingMemory   *WorkingMemory  `json:"working_memory"`   // 工作记忆（如有 session_id）
    ProfileSummary  string          `json:"profile_summary"`  // 用户画像摘要文本
}
```
...
```json
{
  ...
  "data": {
    ...
    "history_summary": "该教师主要教授大学线性代数，偏好互动式教学，常使用案例驱动的方式讲解抽象概念。"
  }
}
```
...
| 操作   | 级联影响                                                                                                               |
| ---- | ------------------------------------------------------------------------------------------------------------------ |
| 删除用户 | → 删除所有 sessions → 删除所有 tasks → 删除 files 记录（`files.user_id ON DELETE CASCADE`）→ 删除 memory_entries → 删除 kb_documents |
