...
就语音代理工具调用兼容性而言，`search_user_memory` 是映射到以下内容的规范工具别名：

- `POST /api/v1/memory/recall`
- 输入/输出仍为 `MemoryRecallRequest` 和 `MemoryRecallResponse`

#### 5.3.2B 记忆纠正/ 撤销 

`**POST /api/v1/memory/corrections**` - 用于纠正/撤销冲突内存的内部接口
...
- 较新的显式用户语句可以覆盖或停用较旧的推断/冲突内存。
- 更正/撤销操作默认保留历史证据（状态转换），而非物理删除。
- 经过更正的活跃内存将成为默认的调用目标，除非显式请求停用/被取代的历史数据。
- 对于 `memory_entry`，更正通常应用于规范的活动结构化记录（通过语义键/上下文进行就地覆盖/停用），同时通过卡片/块/对象层保留历史证据。
- 对于 `memory_card`，更正可通过停用/替换旧卡片并创建/再生更正后的卡片表示来实现。
