...
Voice Agent 通过 `internal/executor` 包统一处理异步动作。`Executor.Execute()` 在独立 goroutine 中执行动作，完成后通过 `ResultCallback` 将结果注入上下文队列：

```go
type ResultCallback func(msg types.ContextMessage)

func (e *Executor) Execute(action protocol.Action, sessionCtx SessionContext, callback ResultCallback) {
    go func() {
        result := e.dispatch(action, sessionCtx, callback)
...
各动作类型（`kb_query`、`web_search`、`ppt_init` 等）在 `executor/` 子文件中实现，均在该 goroutine 内同步执行。
...
| type            | 载荷字段                                             | 说明             |
| --------------- | ------------------------------------------------ | -------------- |
| `task_list_update` | `active_task_id`, `tasks`                        | 任务列表更新（PPT 任务已创建） |
...
│ 3. WebSocket 推送 task_list_update 给前端       │
...
当教师确认需求后，`internal/executor/ppt.go` 的 `executePPTInit` 直接从 `SessionContext` 内联组装 `PPTInitRequest`，无独立方法封装。
...
需求收集完成后，将本次对话异步写入 Memory Service 供后续召回：

```go
go func() {
    clients.ExtractMemory(ctx, MemoryExtractRequest{
        UserID:    userID,
        SessionID: sessionID,
...
> 注：不做需求收集前的预填（prefillFromMemory）。每次从零开始收集，避免旧偏好干扰。
...
type PPTMessageRequest struct {
    ...
    // 扩展字段（Voice Agent 内部使用，复用此结构体传递渲染/导出状态）
    RenderURL   string          `json:"render_url,omitempty"`
    PageIndex   int             `json:"page_index,omitempty"`
    DownloadURL string          `json:"download_url,omitempty"`
...
**`POST /api/v1/kb/retrieve`** — 返回原始 chunk 切片，供 PPT Agent 等需要详细内容的调用方使用

**请求体：** 同 `KBQueryRequest`
...
type WorkingMemory struct {
    ...
    ExtractedElements   map[string]any    `json:"extracted_elements"`
