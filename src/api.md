...
- **scenario 1 (abort)**: the backend stream has not yet emitted the opening `<` of the first `<tool_call>`. the frontend **immediately stops tts playback** and aborts the stream; at the same moment it records how much assistant text had already been spoken (tts-played position). the frontend must maintain the mapping from tts playback progress back to the assistant text so it can compute `interrupted_assistant_text` accurately. no tool call is generated. the assistant message kept in history is truncated at that tts-played position (what the user actually heard), not at the inference position — the tokens between the tts-played position and the inference position are discarded (a few tokens wasted, but few). the first saying words message of the waiting episode gets the `</interrupted>` prefix.
- **scenario 2 (continue)**: the opening `<` of the first `<tool_call>` has already been emitted and tts has **not** finished playing. the frontend **immediately stops tts playback** (signalling it with `tts_finished`, see 0.4) and lets the backend goroutine continue until the **full tool call sequence** is complete and the tool responses are executed. a single `<tool_call>...</tool_call>` may be fully closed, but if the goroutine is still running, there may be more `<tool_call>...</tool_call>` tags following. the frontend must maintain the mapping from tts playback progress back to the assistant text so it can compute `interrupted_assistant_text` accurately. the assistant message kept in history is truncated at the tts-played position and additionally contains the complete tool call sequence. the first saying words message of the waiting episode gets the `</interrupted>` prefix because the user interrupted tts playback.
- **scenario 3 (continue)**: the opening `<` of the first `<tool_call>` has already been emitted and tts has already finished playing. tts playback is not interrupted; the tts line is already complete. the assistant message kept in history contains the complete spoken text plus the complete tool call sequence. the user saying words messages get **no** `</interrupted>` prefix.

in scenarios 2/3 the next inference starts only when **three lines all complete**:

1. the tts line: the frontend must stop tts playback immediately when the user interrupts (scenario 1 and 2). it signals this line's completion by calling `POST /api/v1/voice/tts_finished` (0.4) right after stopping playback — in scenario 3 it calls it right after the interrupt, since tts had already finished. in all three scenarios the tts line completes immediately from the user's point of view; the difference is only whether playback had to be forcibly stopped.
2. the tool call line: only exists in scenarios 2/3. the previous turn's tool call sequence finishes executing and every tool response result is obtained. the results are **not** consumed by a report pass (no post-tool-call tts, unlike the normal flow in 0.2) — they stay pending and go into the next turn's assembly below.
...
**line state (backtracking):** the waiting process has a single state: not complete / complete. while waiting, **every** new interrupt backtracks the **asr line** to its not-complete state and the audio is recollected (the asr line extends; the tts line and the tool call line are only waited on — they belong to the finished previous turn and never backtrack). the process becomes complete only when the **last** speech segment has ended and the other active lines are done; only then does the backend assemble the user messages in the fixed order (`tool responses` when present, then `user saying words`, then `status bar`) and start the next llm inference. in that assembly, the saying words part may be **multiple user messages** (one per speech segment, in speaking order), same as the tool response messages; the status bar is always exactly **one** user message and always last (see 0.2).
...
- the assistant message that goes into the conversation history contains the spoken text truncated at the tts-played position in scenarios 1 and 2, and the complete spoken text in scenario 3. in scenarios 2/3 it additionally contains the complete tool call sequence. in **all** scenarios the assembled context also ends with the status bar user message (`<queue_status>empty|not empty</queue_status>`, the always-last user message, see 0.2).

### 0.2 Post api/v1/voice/vad_end

call this right after vad_end. this is a `POST` request whose response is an sse stream (`content-type: text/event-stream`).
...
- `needs_interrupted_prefix` (bool): `true` when the user interrupted tts playback (scenarios 1 and 2); `false` when tts had already finished playing (scenario 3) or there is no interrupt.
- `interrupted_assistant_text` (string, may be empty): the assistant text that had already been spoken before the interrupt. in scenarios 1 and 2 it is the **truncated** text (only what the user actually heard before tts was stopped); in scenario 3 it is the **complete** spoken text of the previous turn. the backend appends it to the conversation history before starting the next inference so the llm context stays consistent. empty string when there is no text to sync.
...
2. **the waiting episode**: from the moment this vad_end arrives until the llm actually starts the next inference, the process is in a single waiting episode. in scenario 1 there is no tool call line to wait on (the tts line completes immediately when playback is stopped); in scenarios 2/3 the tts line and the tool call line run in parallel. every speech segment transcribed during this episode becomes one user message in the saying words part. if another interrupt happens before the inference starts, the asr line simply extends with a new segment — it is still the same waiting episode, not a recursive new turn.
3. when the waiting episode is complete, assemble the multiple user messages in the fixed order and start the next llm inference:
   - scenario 1: as soon as the latest speech segment has been transcribed (there are no other lines to wait on), assemble `saying words segments (first with `</interrupted>` prefix) → status bar` and start the next inference.
   - scenario 2: when the tool call line is done and the latest speech segment has been transcribed, assemble `tool responses → saying words segments (first with `</interrupted>` prefix) → status bar` and start the next inference.
   - scenario 3: when the tool call line is done and the latest speech segment has been transcribed, assemble `tool responses → saying words segments (no `</interrupted>` prefix) → status bar` and start the next inference.
4. a vad_end that arrives **after** the inference actually starts is an interrupt of the new turn and re-enters the three scenarios recursively.
...
| tool response | `{"type": "tool_response", "text": "all fields are updated"}` | the synchronous result of the tool call just emitted. fed back to the model as a `<tool_response>` block (see the history order note below). |
| turn end | `{"type": "turn_end"}` | signals the end of this assistant turn |

note: the backend executes every tool call synchronously. the stream order is `user_transcript` → `tts` (spoken text) → `tool_call` → `tool_response` → `tool_call` → `tool_response` → ... → **`tts` (report pass)** → `turn_end`. **after all tool calls are emitted, the assistant outputs additional tts text to report the tool results, and no further tool calls are allowed after that.** the report pass only happens in the normal flow: in the interrupted scenarios 2/3 of 0.1, the tool results stay pending and go into the next turn's assembly instead.

history order (status bar technology):

when assembling the conversation for the next inference, the assistant message that goes into the history contains the spoken text **truncated at the tts-played position** if the user interrupted tts playback (scenarios 1 and 2), or the **complete** spoken text if tts had already finished (scenario 3). in scenarios 2/3 it additionally contains the complete tool call sequence. after it, **multiple user messages are assembled in this fixed order: `tool response message(s)` → `user's saying words message` → `status bar message`**. following the qwen3 chat template, each is rendered with `<|im_start|>user` / `<|im_end|>` markers:
...
- `</interrupted>` appears in scenarios 1 and 2 (the user interrupted tts playback). it is the very first thing in the **first** saying words message of the waiting episode. if the user interrupted multiple times during that waiting episode, only the first segment carries `</interrupted>`; the rest are plain speech segments. in scenario 3 the context shows no `</interrupted>` — tts had already finished, so nothing was interrupted.
- if there are no pending tool results, the assembled messages start directly with the saying words message(s); the status bar message is still always last.
...
text-only debug/testing path. skips asr and feeds the text directly into the voice agent stream. this is also a `POST` request that returns an sse stream (`content-type: text/event-stream`).
...
{
    "text": "string",
    "needs_interrupted_prefix": false,
...
- `text` (string): the user's saying words, used directly as the asr transcript.
- `needs_interrupted_prefix` (bool): same meaning as in 0.2; `true` when this turn interrupted tts playback (scenarios 1 and 2), `false` when tts had already finished (scenario 3) or there is no interrupt.
- `interrupted_assistant_text` (string, may be empty): same meaning as in 0.2; the assistant text already spoken before the interrupt (truncated in scenarios 1 and 2, complete in scenario 3). empty when there is no text to sync.

backend processing:

skip asr, use `text` as the transcript, and enter the same three-line waiting / inference flow as 0.2. the backend decides between scenario 1 and scenario 2 by checking whether the previous turn had entered the tool_call phase (i.e. the opening `<` of the first `<tool_call>` had been emitted): if `needs_interrupted_prefix` is `true` and the previous turn had entered the tool_call phase, it is scenario 2; if `needs_interrupted_prefix` is `true` and the previous turn had not entered the tool_call phase, it is scenario 1; if `needs_interrupted_prefix` is `false` and `interrupted_assistant_text` is non-empty, it is scenario 3; otherwise it is a normal new turn. the streamed response format is identical to 0.2.

scenarios expressed by the request fields:

- `needs_interrupted_prefix: true` and the previous turn had entered the tool_call phase → scenario 2. `interrupted_assistant_text` is the **truncated** spoken text.
- `needs_interrupted_prefix: true` and the previous turn had not entered the tool_call phase → scenario 1. `interrupted_assistant_text` is the **truncated** spoken text.
- `needs_interrupted_prefix: false` and `interrupted_assistant_text` is non-empty → scenario 3. `interrupted_assistant_text` is the **complete** spoken text.
- `needs_interrupted_prefix: false` and `interrupted_assistant_text` is empty → a normal new turn with no interrupt.

### 0.4 Post api/v1/voice/tts_finished

the frontend calls this the moment its tts playback finishes or is forcibly stopped. it is the signal that completes the tts line in scenarios 2/3 of 0.1 (and harmlessly in scenario 1, although scenario 1 does not wait on the three lines) — tts playback is frontend-only state, so without this signal the backend would never know when the tts line is done. like vad_start, it is a lightweight sync hook: no audio, no inference.

when to call:

- scenario 1 and 2: right after the frontend stops tts playback (which happens immediately when the user interrupts).
...
- scenario 1 / no active turn: not strictly needed (there is no three-line wait), but calling it is harmless; the backend ignores it if there is no waiting episode.
