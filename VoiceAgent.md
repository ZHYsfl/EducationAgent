...
如果用户在Large LLM生成回答或TTS播放的过程中再次说话打断了，处理方式改为：停止Large LLM的生成（取消当前stream），清空TTS队列（停止播放），但**不丢弃**Large LLM已经生成的任何内容。系统保存的是**原始完整输出**（rawGeneratedTokens），包括`<think>`内部推理和`</think>`后的可见回答，作为一条assistant消息追加到对话历史里并标记为被打断。如果打断时模型还在`<think>`内部（未闭合标签），系统会清理半截标签碎片并补上`</think>`确保历史格式良好。这样下一轮调Large LLM时，模型不仅能看到自己之前说到哪了，还能看到自己想到哪了，可以从断点继续推理，而不是从零重新想。
...
系统通过三层机制保证用户无论怎么说话、打断都不会导致死锁、数据丢失或协程泄漏：

1. **select 防死锁**：所有 channel 发送（audioCh、asrAudioCh、sentenceCh、writeCh、resultCh、tts ch）全部使用 `select` + `ctx.Done()` 或 `default` 保护，任何时刻 context 被取消都不会阻塞。
2. **Mutex 防竞态**：`rawGeneratedTokens`（tokensMu）、session state（stateMu）、draftOutput（draftOutputMu）等共享状态通过互斥锁保护读写安全。
3. **Context 防泄漏**：每轮 pipeline 创建独立 context，打断时一次 `cancel()` 从 session 传播到 LLM stream、TTS 合成、ASR 连接，所有协程统一退出。

覆盖的用户行为场景：

| 场景 | 系统行为 |
|------|----------|
| 正常对话 | 完整走 ASR → Small LLM → draft thinking → LLM → TTS 流程 |
| AI 说话时打断 | OnInterrupt 保存完整输出（含`<think>`推理）到历史，cancel 后重新 Listening |
| AI `<think>` 深度思考时打断 | 同上，自动闭合未完成的`<think>`标签，思考内容不丢失 |
| 连续快速打断（每 2-5 秒一次） | 每次完整走 interrupt → listen → process 循环，历史逐条保存 |
| 用户长时间说话（1-2 分钟） | ASR 持续处理，draft thinker 递增间隔重启积累推理，vad_end 后整体送 LLM |
| 纯语气词 / 环境噪音 | Small LLM 判定 do not interrupt，继续监听不触发处理 |
| AI 刚说完用户立刻接话 | Idle → Listening，正常流程无缝衔接 |
| Listening 中重复 vad_start | 状态机无 Listening→Listening 转换，自动忽略 |

已知限制：频繁打断会导致对话历史快速膨胀（每次打断存一条含`<think>`的 interrupted 消息），长时间高频打断可能撑满模型的上下文窗口。后续可通过 history truncation / summarization 策略优化。
