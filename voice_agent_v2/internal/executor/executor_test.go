...
func (m *mockClients) RecallMemory(ctx context.Context, req types.MemoryRecallRequest) (types.MemoryRecallResponse, error) {
	time.Sleep(10 * time.Millisecond)
	if m.failMemory {
		return types.MemoryRecallResponse{}, context.DeadlineExceeded
	}
