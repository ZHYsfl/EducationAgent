...
7. 遇到冲突问题时，基于用户原话直接通过 @{ppt_mod|raw_text:用户原话|user_distance:int} 反馈，不额外调用冲突选择工具。`
...
func (p *Pipeline) postProcess(_ context.Context, userText string, _ []protocol.Action) {
	if p.clients == nil {
		return
	}
	...
	pending := make(map[string]PendingQuestion, pendingCount)
	for cid, pq := range p.session.PendingQuestions {
		pending[cid] = pq
	}
	p.session.pendingQMu.RUnlock()

	if pendingCount == 0 || !isMeaningfulConflictReply(userText) {
		return
	}
	...
	contextID, ok := pickPendingConflict(userText, activeTask, viewingPage, pending)
	if !ok {
		return
	}

	pq, ok := p.session.ResolvePendingQuestion(contextID)
	if !ok {
		return
	}

	taskID := strings.TrimSpace(pq.TaskID)
	if taskID == "" {
		taskID = activeTask
	}
	if taskID == "" {
	...
	req := PPTFeedbackRequest{
		...
		RawText:       strings.TrimSpace(userText),
		Intents:       nil,
	}

	if err := p.clients.SendFeedback(context.Background(), req); err != nil {
		log.Printf("[post] resolve conflict feedback failed: task_id=%s context_id=%s err=%v", taskID, contextID, err)
		return
	}

	p.EnqueueContext(ContextMessage{
		ID:        newID("ctx_"),
		EventType: "ppt_mod",
		Priority:  "normal",
		Content:   "已提交冲突解决反馈",
...
func pickPendingConflict(userText, activeTaskID, viewingPageID string, pending map[string]PendingQuestion) (string, bool) {
	userText = strings.TrimSpace(userText)
	if userText == "" || len(pending) == 0 {
		return "", false
	}

	lowerText := strings.ToLower(userText)
	for cid := range pending {
		if strings.Contains(lowerText, strings.ToLower(cid)) {
			return cid, true
		}
	}

	if !isMeaningfulConflictReply(userText) {
		return "", false
	}
	...
	for _, id := range ids {
		candidates = append(candidates, candidate{id: id, q: pending[id]})
	}

	if activeTaskID != "" {
		filtered := make([]candidate, 0, len(candidates))
		for _, c := range candidates {
			if c.q.TaskID == activeTaskID {
	...
	tie := false
	for _, c := range candidates {
		score := scoreConflictMatch(userText, c.q.QuestionText)
		if activeTaskID != "" && c.q.TaskID == activeTaskID {
			score += 2
		}
		if viewingPageID != "" && c.q.PageID == viewingPageID {
			score += 1
		}
...
func isMeaningfulConflictReply(userText string) bool {
	...
	if (strings.Contains(t, "继续") || strings.Contains(t, "下一页")) && len(runes) <= 8 {
		return false
	}
	return true
}

func scoreConflictMatch(userText, question string) int {
	u := normalizeForMatch(userText)
	...
	score := 0
	if strings.Contains(q, u) || strings.Contains(u, q) {
		score += 4
	}

	uShingles := buildShingles([]rune(u), 2)
	qShingles := buildShingles([]rune(q), 2)
	for key := range uShingles {
		if _, ok := qShingles[key]; ok {
			score++
		}
	}
	return score
...
func buildShingles(runes []rune, n int) map[string]struct{} {
